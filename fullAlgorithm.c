...
void roiPrint(FILE *inputFile,int roiWidth, int roiHeight,int whichDigit){
    ...
    int digitOffset=(whichDigit-1)*digitWidth;//digit offset from start of roi is width of a digit*the digit needed-1
...
int main(void){
    ...
    int i;
    for (i=1; i<4; i++) {
        roiPrint(source, roiWidth,roiHeight,i);
        //digit detection
        reshape();
        readfinalB1L1();
        readfinalB1L2();
    ...
    /*
    roiPrint(source, roiWidth,roiHeight,3);
    //digit detection
    reshape();
    readfinalB1L1();
    readfinalB1L2();
